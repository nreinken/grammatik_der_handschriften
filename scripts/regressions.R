...
checkOutliers <- function(model, threshold = 100 * mean(cooks.distance(model),
    na.rm = TRUE)) {
...
checkAssumptions <- function(model, data, generate_plot = TRUE) {
    ...
    if (generate_plot) {
        plot <- mydata %>%
            ggplot(aes(x = logit, y = predictor.value)) + geom_point(size = 0.5,
            alpha = 0.5) + geom_smooth(method = "loess", se = FALSE) + facet_wrap(~predictors,
...
paraModel <- function(character, data) {
    ...
    stopifnot(is.data.frame(data), all(c("code", "letter", "person_ID", "word_index",
        "letter_freq") %in% colnames(data)))
    ...
    # set up model
    model <- nnet::multinom(formula = formula, data = train.data, MaxNWts = 9999,
        model = T, trace = F)
    # evaluate model
...
getCoefs <- function(model, varName, fileName) {
    ...
    write.csv2(coefs, file.path("results", paste0("coefficients_", fileName,
        ".csv")), row.names = FALSE)
