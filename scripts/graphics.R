...
plot_assoc <-
           ...
           png = TRUE)
  {
    ...
    p <- ggplot(resids, aes(x = resids[, 2], y = Freq, fill = color)) +
      geom_bar(
        position = "dodge",
        stat = "identity",
        width = resids$count / sum(resids$count)
      ) +
      scale_fill_grey() +
    ...
    {
      ggsave(
        paste0("graphs/assoc_", x_title, "_", y_title, ".eps"),
        width = 3,
        height = 3
      )
    }
    ...
    {
      ggsave(
        paste0("graphs/assoc_", x_title, "_", y_title, ".png"),
        width = 3,
        height = 3
      )
    }
                       ...
                       png = T)
{
  ...
  if (eps)
  {
    ggsave(paste0("graphs/coefs_", name, ".eps"),
           width = 4,
           height = 8)
  }
  if (png)
  {
    ggsave(paste0("graphs/coefs_", name, ".png"),
           width = 4,
           height = 8)
  }
