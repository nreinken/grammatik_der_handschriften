...
# Form and function of <e> ==== load data
d_e <- data.loadData(whichColumns = c("code", "e_func"), letter = "e", removeUpperCase = T,
    removeUnrecognisable = T, removeWaZ = F, removeWordEnds = F)
...
d_dist$gsyll_struc <- factor(d_dist$gsyll_struc, levels = c("ONS", "NUC",
    "KEY", "CODA", "EXTRA"))
...
letter_pairs <- list(c("a5", "o3"), c("f7", "t3"), c("f9", "h6", "l2", "t4"),
    c("h1", "k3"), c("h5", "l1"), c("n1", "u1"), c("r2", "v1"))
...
for (pair in letter_pairs) {
    ...
    # we need to cast 'code' to characters for now, so we can add a new
    # factor level more easily
...
# clean up
rm(d_dist, d_dist_temp, d_dist_temp2, letter_pairs, form, letter, letter_recs,
    pair)



# Double Consonants load data
d_dbl <- data.loadData(whichColumns = c("letter_rec", "code", "double_cons",
    "double_index"), removeWaZ = F, removeWordEnds = F, removeUpperCase = T,
...
# check if there are form differences between the first and second part
# of the letter
...
for (double_consonant in double_consonants) {
    ...
    cont_test(d_dbl_diffs, x_title = paste0("double_cons_position", double_consonant),
        y_title = "form")
}
...
for (level in levels(d_dbl$double_cons)) {
    ...
    for (index in 1:nrow(d_dbl_forms)) {
        ...
        if (case$double_index == 1) {
            # go to the next case (that is the second part of the double
            # consonant) and check whether it has the same shape
...
# lettershapes in diphthongs ==== load data
d_diph <- data.loadData(whichColumns = c("code", "letter_rec", "phon_class",
    "gsyll_struc"), letter = c("a", "e", "i", "o", "u", "y"), removeUpperCase = T,
    removeUnrecognisable = T)
...
for (letter in secondParts) {
    ...
    cont_test(d_diph_single, x_title = "diphthong", y_title = paste0(letter,
        "_form"))
}
# clean up
rm(letter, secondParts, d_diph, d_diph_single)

# lettershapes in key position ==== load data
...
d_key$gsyll_struc <- plyr::revalue(d_key$gsyll_struc, c(ONS = "not KEY", NUC = "not KEY",
    EXTRA = "not KEY", CODA = "not KEY"))
...
# Phonographic structures ==== lettershape and vowel tension ==== load
# data
d_vowels <- data.loadData(whichColumns = c("code", "phon_class", "phon_vtension",
    "letter_rec"), letter = vowels, removeWaZ = F, removeWordEnds = F, removeUpperCase = T,
    removeUnrecognisable = T)
...
for (letter in vowels) {
    ...
    # run contingency tests
    cont_test(d_singleVowel, x_title = "tension", y_title = paste0("form_",
        letter))
}
# clean up
...
d_cons <- data.loadData(whichColumns = c("code", "phon_class", "phon_cvoiced",
    "letter_rec"), letter = consonants, removeWaZ = F, removeWordEnds = F,
...
for (letter in consonants) {
    ...
    # run contingency tests
    cont_test(d_singleCons, x_title = "voicedness", y_title = paste0("form_",
        letter))
}
# clean up
rm(d_singleCons, letter, consonants, d_cons)
# Case study: Lettershapes and voicedness for each text ==== load data
d_text <- data.loadData(whichColumns = c("person_ID", "letter_rec", "code",
    "phon_cvoiced"), letter = c("b", "d", "g", "s"), removeWaZ = F, removeWordEnds = F,
    removeUpperCase = T, removeUnrecognisable = T)
...
# group data by texts
d_text <- group_by(d_text, person_ID) %>%
    group_split()
...
for (text in d_text) {
    ...
    for (letter in voiced_letters) {
        ...
        print(paste0("Letter ", letter))
        if (nlevels(d_subset$code) <= 1 || nlevels(d_subset$phon_cvoiced) <=
            1) {
            print("No occurence of letter, only one letter form or no change in voicedness; skipping letter.")
        } else {
            # run contingency tests
            cont_test(d_subset, x_title = "voice", y_title = paste0(text$person_ID[1],
                "_", letter))
        }
    }
}
...
# <h> shape and junctions #### load data
d_h <- data.loadData(whichColumns = c("code", "junc_border_before", "junc_border"),
    letter = "h", removeWaZ = T, removeUnrecognisable = F, removeUpperCase = F,
    removeWordEnds = F)
...
# load data
d_complex <- data.loadData(whichColumns = c("junc_border", "graph_complexity",
    "letter_rec", "code", "graph_complexity_both"))

# rename some factor levels
d_complex$junc_border <- plyr::revalue(d_complex$junc_border, c(`TRUE` = "separation",
    `FALSE` = "connection"))

# save dataset with pseudo-complex graphemes and with letters for later
# use
d_complex_pseudo <- d_complex
d_complex_withLetter <- d_complex

# rename some factor levels
d_complex$graph_complexity <- plyr::revalue(d_complex$graph_complexity, c(`pseudo-rh` = "FALSE",
    `pseudo-th` = "FALSE", `pseudo-st` = "FALSE"))
...
# remove letter_rec, code, and graph_complexity_both where they're not
# needed
...
newValues <- c(ch = "complex", rh = "complex", ck = "complex", pf = "complex",
    ph = "complex", qu = "complex", st = "complex", th = "complex", el = "not complex",
    ng = "not complex", `FALSE` = "not complex")
d_complex_binary$graph_complexity <- plyr::revalue(d_complex_binary$graph_complexity,
    newValues)
...
# Compare complex graphemes individually ==== <rh> is too rare, so it's
# omitted
...
# Analyse <sch> ==== select <sch> cases
d_complex_sch <- filter(d_complex_withLetter, graph_complexity %in% c("sch",
    "FALSE"))
...
d_complex_ng$graph_complexity <- ifelse(d_complex_ng$graph_complexity == "FALSE",
    "not <ng>", "<ng>")
...
d_complex_el$graph_complexity <- ifelse(d_complex_el$graph_complexity == "FALSE",
    "not <el>", "<el>")
...
# Are there special letter forms in complex graphemes? ==== remove all
# letters not occuring in complex graphemes or not regognizable
...
d_complex_letterForms$graph_complexity_both <- plyr::revalue(d_complex_letterForms$graph_complexity_both,
    c(ch = "complex", ck = "complex", `FALSE` = "not complex", th = "complex",
        ng = "complex", el = "complex"))
d_complex_letterForms <- droplevels(d_complex_letterForms)

# since the frequencies are dependent on the overall letter frequency, I
# need to test for each letter individually
for (letter in test_letters) {
...
# and that's the analysis of complexe graphemes clean up
rm(d_complex)
rm(d_complex_letter)
...
# Syllabic structures ==== phonographic syllable boundaries ==== load
# data
psyll <- data.loadData(whichColumns = c("junc_border", "psyll_border"), removeWaZ = T,
...
# graphic syllable boundaries ==== load data
gsyll <- data.loadData(whichColumns = c("junc_border", "gsyll_border"), removeWaZ = T,
    removeWordEnds = T, removeUpperCase = F, removeUnrecognisable = F)
...
# Lettershape in syllable positions ==== test single letters ==== load
# data
...
# sort factor levels
d_syll$gsyll_struc <- factor(d_syll$gsyll_struc, c("ONS", "NUC", "KEY", "CODA",
    "EXTRA"))
...
for (letter in letters) {
    ...
    # run tests
    cont_test(d_subs, x_title = "syllablePosition", y_title = paste0(letter,
        "-shape"))
}
# clean up
rm(d_subs, d_syll)
# test reduced letters against other shapes ===== define reduced forms
reduced_forms <- c("a5", "e3", "f6", "f7", "f8", "f9", "g5", "g6", "h5", "g6",
    "k8", "o3", "r4", "r5", "s5", "t5")
...
for (letter in reduced_letters) {
    ...
    # sort factor levels
    d_reds$gsyll_struc <- factor(d_reds$gsyll_struc, c("ONS", "NUC", "KEY",
        "CODA", "EXTRA"))
    ...
    # run contingency tests
    cont_test(d_reds, x_title = "syllablePosition", y_title = paste0("reduced_",
        letter))
}
rm(forms, letter, reduced_forms, reduced_letters, d_reds)

# Graphematic feet ==== Check if <e> form depends on syllable type ====
d_esyll <- data.loadData(whichColumns = c("code", "gsyll_type"), letter = "e",
    removeWaZ = F, removeWordEnds = F, removeUpperCase = T, removeUnrecognisable = T)
...
# load data
d_gfoot <- data.loadData(whichColumns = c("junc_border", "gfoot_border", "gfoot"),
    removeUpperCase = F, removeUnrecognisable = F, removeWaZ = T, removeWordEnds = T)
...
# choose only trochees from trochee-only-dataset load data
d_gfoot_onlyTro <- data.loadData(whichColumns = c("junc_border", "gfoot_border_noDac",
    "gfoot_noDac"), removeUpperCase = F, removeUnrecognisable = F, removeWaZ = T,
    removeWordEnds = T)
...
# Reduced <e> at morphological positions load data
eRed_morph <- data.loadData(whichColumns = c("gsyll_type", "code", "morph_cat"),
    letter = "e", removeWaZ = F, removeWordEnds = F, removeUpperCase = T,
    removeUnrecognisable = T)

# only use cases with the <e> in inflexion affixes and in lexemes and
# reduced syllables
...
# rename factor levels
eRed_morph$morph_cat <- plyr::revalue(eRed_morph$morph_cat, c(LEX = "pseudo affix / morphological remnant",
    FLEX = "inflexion affix"))
...
# Morphographic structures ==== Lettershapes at morphological positions
# ==== load data
d_morph <- data.loadData(whichColumns = c("letter_rec", "code", "morphographic"),
    removeWaZ = F, removeWordEnds = F, removeUpperCase = T, removeUnrecognisable = T)
...
for (letter in letters) {
    ...
    # run analysis
    cont_test(d_morph_single, x_title = "morphographic", y_title = paste0(letter,
        "_form"))
}
...
# Morpheme processes ==== load data
d_morph_type <- data.loadData(whichColumns = c("junc_border", "morph_border_type"),
    removeWaZ = T, removeWordEnds = T, removeUpperCase = F, removeUnrecognisable = F)
# refactor the morphological processes
newValues <- c(`LEX-FLEX` = "inflection", `LEX-SFX` = "suff.", `LEX-LEX` = "comp.",
    `PFX-LEX` = "pref.", `SFX-FLEX` = "inflection", `FLEX-FLEX` = "inflection",
    `FLEX-LEX` = "inflection", `FLEX-SFX` = "suff.", `FUG-FLEX` = "inflection",
    `FUG-LEX` = "comp.", `FUG-PFX` = "comp.", `FUG-SFX` = "suff.", `LEX-FUG` = "comp.",
    `LEX-PFX` = "comp.", `LEX-ZFX` = "circumf.", `PFX-FLEX` = "pref.", `PFX-PFX` = "pref.",
    `PFX-SFX` = "comp.", `SFX-FUG` = "comp.", `SFX-LEX` = "comp.", `SFX-PFX` = "comp.",
    `SFX-SFX` = "suff.", `ZFX-LEX` = "circumf.", n.V. = "none")
d_morph_type$morph_border_type <- plyr::revalue(d_morph_type$morph_border_type,
    newValues)
d_morph_type$morph_border_type <- factor(d_morph_type$morph_border_type, levels = c("none",
    "inflection", "pref.", "suff.", "comp.", "circumf."))
rm(newValues)
...
# Form and function of <h> ==== load data
d_h <- data.loadData(whichColumns = c("code", "h_func"), letter = "h", removeUpperCase = T,
    removeUnrecognisable = T, removeWaZ = F, removeWordEnds = F)
