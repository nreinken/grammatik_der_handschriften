...
d_letters <-
  data.loadData(
    whichColumns = c("junc_border", "next_letter", "prev_letter"),
    removeWaZ = T,
    removeWordEnds = T,
    removeUpperCase = T,
    removeUnrecognisable = F
...
full.model <-
  glm(formula, data = d_letters.train, family = binomial())
...
coefs <-
  getCoefs(best.model, varName = "next_letter", fileName = "letters")
...
coefs_ext <-
  mutate(coefs_ext, pos = ifelse(coefs > 0, coefs + 0.7, coefs - 0.7))
...
#load data
d_forms <- data.loadData(
  whichColumns = c("code", "junc_border"),
  removeWaZ = T,
  removeWordEnds = T,
  removeUpperCase = T,
  removeUnrecognisable = T
...
full.model <-
  glm(formula, data = d_forms.train, family = binomial())
...
coefs <-
  getCoefs(best.model, varName = "code", fileName = "letterforms")
...
coefs_ext <- mutate(coefs_ext, name = names)
coefs_ext <-
  mutate(coefs_ext, pos = ifelse(coefs > 0, coefs + 1.5, coefs - 1.5))
...
d_bigr <-
  data.loadData(
    whichColumns = c("junc_border", "bigramm_next"),
    removeWaZ = T,
    removeWordEnds = T,
    removeUpperCase = T,
    removeUnrecognisable = F
...
d_bigr <-
  droplevels(filter(d_bigr,!bigramm_next %in% low_freqs$Var1))
rm(freqs, low_freqs)
...
coefs <-
  getCoefs(best.model, varName = "bigramm_next", fileName = "bigrams")
